...
namespace Dx12
{
	...
	public:
		/* コンストラクタ */
		Descriptor(const D3D12_DESCRIPTOR_HEAP_TYPE& type, const std::uint32_t& rsc_num, 
	...
	public:
		...
		/* UAVの生成(バッファー) */
		bool CreateUnorderAccessView(Resource* rsc, const std::uint32_t& element_num);
		/* UAVの生成(テクスチャ) */
	...
	private:
		...
		ID3D12DescriptorHeap* CreateDescriptorHeap(const D3D12_DESCRIPTOR_HEAP_TYPE& type, 
			const std::uint32_t& rsc_num, const D3D12_DESCRIPTOR_HEAP_FLAGS& flag);
